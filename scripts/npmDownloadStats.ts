...
async function getPackageDownloads(packageName: string): Promise<number | null> {
  try {
    const response = await fetch(`https://api.npmjs.org/downloads/point/last-month/${packageName}`);
    const data = await response.json();
    return data.downloads;
  } catch (error) {
    console.error(`Failed to fetch download stats for package ${packageName}: `, error);
    return null;
  }
}

async function sortPackagesByDownloads(packages: string[]): Promise<PackageDownloadStats[] | null> {
  try {
